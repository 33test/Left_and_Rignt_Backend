...
router.get('/cartQuery', (req, res) => {
  const userId="10001";
  const query = 'SELECT p.*,pi.image_path,c.quantity FROM products p JOIN cart c ON p.product_id = c.product_id LEFT JOIN product_images pi ON p.product_id = pi.product_id AND pi.order_sort = 1 AND image_type="main" WHERE  c.user_id = ?;';
